...
// postgresDriverForTest returns a connected PostgresDriver. sql.Open is lazy
// for this driver (it doesn't dial), so Connect/Close can be exercised
// without a live server; the DB-hitting methods below are expected to fail
// since nothing is actually listening on the configured port.
func postgresDriverForTest(t *testing.T) db.Driver {
	t.Helper()
...
func TestPostgresDriver_ConnectClose(t *testing.T) {
